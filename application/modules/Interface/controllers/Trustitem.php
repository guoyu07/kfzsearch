 ...
 * @date   2015年12月22日12:00:29
 */
class TrustitemController extends BaseInterfaceController
{
    ...
    {
        parent::init();
        $this->searchModel = new TrustElasticModel();
    }
...
//        file_put_contents('/tmp/kfzsearch.log', var_export($params, true). "\n", FILE_APPEND);
        ...
        if(is_array($params) && !empty($params)){
            ...
            //具体调用搜索方法  非必传
            $action         = isset($params['action']) && $params['action'] ? $params['action'] : 'getTrustItems';
            if(empty($requestParams)) {
                $this->result['failDesc'] = 'the requestParams is invalid';
                $this->response($this->result);
                exit;
            }
            if(empty($bizFlag) || !in_array($bizFlag, array('trust_item'))) {
